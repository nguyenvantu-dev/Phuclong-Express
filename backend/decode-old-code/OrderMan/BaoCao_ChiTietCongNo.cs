...
public class BaoCao_ChiTietCongNo : Page
{
	...
	protected DropDownList ddTuKy;

	protected DropDownList ddDenKy;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		if (!base.IsPostBack)
		{
			...
			if (!string.IsNullOrEmpty(value) && int.TryParse(Page.Request.QueryString["kid"], out var result))
			{
				...
				ddTuKy.Items.FindByValue(result.ToString()).Selected = true;
				ddDenKy.Items.FindByValue(result.ToString()).Selected = true;
	...
	private void LoadDanhSachKy()
	{
		try
		{
			...
			ddTuKy.DataSource = dBConnect.LayDanhSachKyByTinhTrang(DaDong: true);
			ddTuKy.DataBind();
			ddTuKy.Items.Insert(0, new ListItem("--Không xác định--", "-1"));
			ddDenKy.DataSource = dBConnect.LayDanhSachKy();
			ddDenKy.DataBind();
		}
		catch
		{
	...
	private void LoadBaoCaoChiTietCongNo()
	{
		...
		{
			DBConnect dBConnect = new DBConnect();
			DataSet dataSet = dBConnect.BaoCaoChiTietCongNo(druser.SelectedItem.Value, Convert.ToInt32(ddTuKy.SelectedValue), Convert.ToInt32(ddDenKy.SelectedValue), pageSize, pageNum);
	...
	protected void btExportToExcelAllWithFilter_Click(object sender, EventArgs e)
	{
		...
		dataSet = dBConnect.BaoCaoChiTietCongNo(druser.SelectedItem.Value, Convert.ToInt32(ddTuKy.SelectedValue), Convert.ToInt32(ddDenKy.SelectedValue), 10000000, 1);
		int num = 0;
